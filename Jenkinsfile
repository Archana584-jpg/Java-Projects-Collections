        ...
        stage('Checkout') {
            steps {
                ...
                script {
                    ...
                    echo "PR: ${CHANGE_ID} | Commit: ${env.COMMIT_HASH} | Repo: ${env.REPO_URL}"
                }
            }
        }
                ...
                script {
                    ...
                    if (changeCount > 0) {
                        env.CHANGED_FILES = sh(
                            script: '''
                                git diff --name-only origin/${CHANGE_TARGET}...HEAD | grep -E '\\.(java|html|js|py)$' | tr '\\n' ','
                            ''',
                            returnStdout: true
                        ).trim().replaceAll(',$', '')
            ...
            steps {
                sh '''
                    if [ ! -f "${SONAR_SCANNER_HOME}/bin/sonar-scanner" ]; then
                        ...
                        echo "Scanner ready at ${SONAR_SCANNER_HOME}"
                    else
            ...
            steps {
                script {
                    ...
                    if (projectExists == '1') {
                        ...
                        echo "Project ${SONAR_PR_KEY} already exists, will reuse it"
                    }
                }
            }
        }
            ...
            steps {
                timeout(time: 10, unit: 'MINUTES') {
                    sh '''
                        ...
                        echo "Scan complete, waiting for SonarQube to process..."
                    '''
                }
            }
        }
                        ...
                        def qgStatus = sh(
                            script: '''
                                for i in {1..18}; do
                                    ...
                                    echo "Attempt $i/18: waiting for analysis..."
                                    sleep 10
                                done
                                echo "TIMEOUT"
                            ''',
        ...
        stage('Preserve Project on Fail') {
            ...
            steps {
                script {
                    ...
                    echo "Preserving project ${SONAR_PR_KEY} for manual review at:"
                    echo "${SONAR_HOST_URL}/dashboard?id=${SONAR_PR_KEY}"
        ...
        stage('Report to GitHub') {
            ...
            steps {
                script {
                    ...
                    } else if (env.QG_STATUS == 'ERROR') {
                        state = 'failure'
                        desc = 'Quality gate failed - review issues'
                        targetUrl = "${SONAR_HOST_URL}/dashboard?id=${SONAR_PR_KEY}"
                    } else {
                        state = 'error'
                        desc = 'Unknown quality gate status'
                        targetUrl = "${SONAR_HOST_URL}/dashboard?id=${SONAR_PR_KEY}"
                    }
                    
                    echo "Posting to GitHub: state=${state}, desc=${desc}"
        ...
        failure {
            script {
                if (env.CHANGE_ID != null && env.CHANGE_ID != '') {
                    echo "Pipeline failed — reporting error to GitHub"
                    sh '''
                        curl -X POST \
                            -H "Authorization: token ${GITHUB_TOKEN}" \
                            -H "Content-Type: application/json" \
